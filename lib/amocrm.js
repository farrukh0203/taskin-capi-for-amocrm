...
async function getLeadDetails(leadId) {
  ...
  // fb_lead_id: "Facebook №1312378530960527" → "1312378530960527"
  const nameMatch = lead?.name?.match(/[#№](\d+)/);
  const fbLeadId = nameMatch ? nameMatch[1] : null;
  console.log(`Lead ${leadId}: fb_lead_id=${fbLeadId ?? 'topilmadi'}`);

  if (!contacts.length) {
    console.log(`Lead ${leadId}: kontakt topilmadi`);
    return { fn: null, ln: null, phone: null, fbLeadId };
  }
